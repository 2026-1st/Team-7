...
## 각 지표별 1등 조합으로 모델 학습
# ROC_AUC와 PR_AUC는 1등 모델이 동일해서 총 5개
top_combinations = [
...
# 결과를 저장할 데이터프레임 생성
importance_comparison_df = pd.DataFrame({"Feature": feature_names})

# 5개의 최적 모델을 루프 돌며 통째로 재학습시키고 가중치를 뽑아 합치기
valid_results=[]
...
for comb in top_combinations:
    ...
    # 해당 모델이 계산한 변수별 중요도를 컬럼으로 추가
    importance_comparison_df[comb["name"]] = model.feature_importances_.round(4)

# #지표 별 1등 모델의 중요도 기준으로 정렬
# 모든 변수가 한눈에 다 보이도록 판다스 행 제한 해제 후 출력
pd.set_option('display.max_rows', 100)
print("[1] Precision 1등 모델 기준 변수 정렬")
display(importance_comparison_df[['Feature', 'Model_Precision_Top (Index 34)']].sort_values(by='Model_Precision_Top (Index 34)', ascending=False).head(10))

print("\n[2] Recall 1등 모델 기준 변수 정렬")
display(importance_comparison_df[['Feature', 'Model_Recall_Top (Index 68)']].sort_values(by='Model_Recall_Top (Index 68)', ascending=False).head(10))

print("\n [3] F1-Score 1등 모델 기준 변수 정렬")
display(importance_comparison_df[['Feature', 'Model_F1_Top (Index 70)']].sort_values(by='Model_F1_Top (Index 70)', ascending=False).head(10))

print("\n [4] ROC-AUC, PR-AUC 1등 모델 기준 변수 정렬")
display(importance_comparison_df[['Feature', 'Model_ROCAUC_PRAUC_Top (Index 31)']].sort_values(by='Model_ROCAUC_PRAUC_Top (Index 31)', ascending=False).head(10))

print("\n [5] LogLoss 1등 모델 기준 변수 정렬")
display(importance_comparison_df[['Feature', 'Model_LogLoss_Top (Index 3)']].sort_values(by='Model_LogLoss_Top (Index 3)', ascending=False).head(10))
