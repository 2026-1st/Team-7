...
📦 Team-7
 ...
 ┃ ┃ ┗ 📜 lgbm_model_FE.ipynb
 ┃ ┣ 📂 Logistic
 ┃ ┃ ┗ 📜 logistic_feature_engin.ipynb
 ┃ ┣ 📂 RandomForest
 ┃ ┃ ┗ 📜 random_feature_engin.ipynb
 ┃ ┣ 📂 TabNet
 ┃ ┃ ┗ 📜 TabNet.ipynb
 ┃ ┣ 📂 XGBoost
 ┃ ┃ ┗ 📜 XGBoost.ipynb
 ...
 ┃ ┣ 📜 preprocessing_code_numcompanies.py
 ┃ ┣ 📜 preprocessing_fe.py
 ┃ ┗ 📜 feature_eng.py (파생 변수 생성 코드)
 ┗ 📜 README.md
```
## 3. 실행 방법과 환경 (How to Run & Environment Notes)
**실행 방법:** LightGBM, 로지스틱 회귀와 랜덤 포레스트는 로컬 환경에서 사용되었던 코드이기에 데이터 경로를 클론한 파일 위치를 기반으로 변경하고 사용해야 합니다. XGBoost와 TabNet은 Colab 상에서 사용된 코드이기에 셀 단위로 하나씩 실행하면 재현 가능합니다.
...
데이터 누수 방지: K-Fold나 Hold-out 분할 이전에 오버샘플링(SMOTE 등)을 적용하지 않고, 층화 추출(Stratify)을 엄격하게 유지했습니다.
